 ...
 */
float zoomFactor(types::control map) {
	auto COORDS_CENTER = types::vector2(50, 50);
	auto COORDS_TL = types::vector2(0, 100);
	auto COORDS_BR = types::vector2(100, 0);
	...
	while (abs(widthInPixels - RESOLUTION) > 0.1)
	{
		...
		zoomFactor = zoomFactor / (RESOLUTION / w);
	}
...
void mapTileGenerator(int levelOfDetail) {
	...
	std::this_thread::sleep_for(std::chrono::seconds(5));
	{
		...
		float zoomFactor = zoomFactor(map);
